...
structure CarlitzModule (q : ℕ) [Fact q.Prime] where
  ...
  /-- Additivity: φ_{a+b} = φ_a + φ_b (pointwise) -/
  add_action : ∀ (a b : A q) (x : L q), action (a + b) x = action a x + action b x
  /-- Multiplicativity: φ_{ab} = φ_a ∘ φ_b -/
  mul_action : ∀ (a b : A q) (x : L q), action (a * b) x = action a (action b x)

namespace CarlitzModule

variable {q : ℕ} [Fact q.Prime] (φ : CarlitzModule q)
...
/-- The Carlitz module defines a ring homomorphism (additive part) -/
theorem is_ring_hom (a b : A q) (x : L q) : φ.action (a + b) x = φ.action a x + φ.action b x :=
  φ.add_action a b x
