...
-- L q = SeparableClosure (K q) has noncomputable Algebra/Module instances that Lean's
-- instance synthesis cannot find automatically for IntermediateField (K q) (L q).
-- We register the Module instance explicitly so that FiniteDimensional and finrank work.
noncomputable instance (E : IntermediateField (K q) (L q)) : Module (K q) ↥E :=
  Algebra.toModule

/-- The cyclotomic function field K(Λ_M) generated by M-division points -/
noncomputable def CyclotomicField (M : A q) (φ : CarlitzModule q) : IntermediateField (K q) (L q) :=
